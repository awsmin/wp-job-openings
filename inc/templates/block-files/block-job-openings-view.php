...
if ( $placement === 'top' ) {
	...
	?>
			<div <?php echo $wrapper_attrs; ?>>
				<?php if ( $show_filter ) { ?>
					<div class="awsm-b-filter-wrap<?php echo ( is_admin() || ( defined( 'REST_REQUEST' ) && REST_REQUEST ) ) ? '' : ' awsm-selectric-loading'; ?><?php echo esc_attr( apply_filters( 'awsm_jobs_filter_wrap_extra_class', '' ) ); ?>">
						<?php
						/**
				...
				if ( ! $show_filter ) {
					...
					if ( ! empty( $extra_inside ) ) {
						echo '<div class="awsm-b-filter-wrap-extra' . esc_attr( apply_filters( 'awsm_jobs_filter_wrap_extra_class', '' ) ) . '">' . $extra_inside . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					}
				}
				?>
				<?php
